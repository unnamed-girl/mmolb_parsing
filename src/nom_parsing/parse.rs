...
fn weather_wither<'parse, 'output: 'parse>(parsing_context: &'parse ParsingContext<'parse>) -> impl MyParser<'output, ParsedEventMessage<&'output str>> + 'parse {
    let weather_reflection = |input| {
        ...
        let (input, (placed_player_str, corrupted, )) = alt((
            parse_terminated(" resisted the effects of the 🥀 Wither while Effloresced").map(|n| (n, WitherResult::ResistedEffloresced)),
            parse_terminated(" resisted the effects of the 🥀 Wither with 🦠 Immunity").map(|n| (n, WitherResult::ResistedImmune)),
            parse_terminated(" resists the effects of the 🥀 Wither").map(|n| (n, WitherResult::Resisted)),
            parse_terminated(" resisted the effects of the 🥀 Wither").map(|n| (n, WitherResult::Resisted)),
            parse_terminated(" was Corrupted by the 🥀 Wither").map(|n| (n, WitherResult::Corrupted)),
            parse_terminated(" is Corrupted by the 🥀 Wither").map(|n| (n, WitherResult::Corrupted)),
        )).parse(input)?;
        ...
        Ok((input, ParsedEventMessage::WeatherWither { team_emoji, player, corrupted, contained }))
    };
