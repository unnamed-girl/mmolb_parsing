...
use clap::Parser;
use mmolb_parsing::{
    enums::{Attribute, EquipmentEffectType, ItemName, Position, Slot},
...
fn mmolb_fetch<'a, T: Fetchable + DeserializeOwned>(
    client: &'a Client,
    id: &str,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for player in team.players {
        ...
        let items = items
            .into_iter()
            .flat_map(UnderstoodItem::try_from)
            .collect::<Vec<_>>();
        ...
        for item in items {
            ...
            for (attribute, value) in item.effects {
                ...
                match (fielder_type, attribute) {
                    ...
                    (
                        FieldPlace::Pitcher,
                        Attribute::Reaction | Attribute::Acrobatics | Attribute::Patience,
                    ) => {
                        badness += value;
                    ...
                    (FieldPlace::Outfield, Attribute::Reaction) => {
                        badness += value;
                        criticisms
                            .push(format!("Outfielder makes poor use of +{value} {attribute}"))
                    }
                    _ => {
                        if is_fielder {
                            goodness += value;
                ...
                if is_fielder && fielder_type == FieldPlace::DH {
                    badness += value;
                    criticisms.push(format!(
                        "Designated hitter cannot make use of +{value} {attribute}"
                    ))
                }
            ...
            if fielder_type == FieldPlace::Pitcher && batter > pitcher {
                ...
                criticisms.push(format!(
                    "Pitcher holding item with net +{diff} batting attributes"
                ));
            } else if fielder_type != FieldPlace::Pitcher && batter < pitcher {
                let diff = pitcher - batter;
                criticisms.push(format!(
                    "Non pitcher holding item with net +{diff} pitcher attributes"
                ));
            }
